...
Backend implementation of the Quantum Readiness assessment app, aligned with `app_definition.md` and the shell-and-core architecture.
...
- **Layer 3: Tools (`tools/`)**  
  ...
  - `quantum_data_collector` (multi-turn assessment questions with validation and clarification)
...
2. Create `.env` from `env.example` and fill your key:
```bash
cp env.example .env
```
...
```env
# LLM Configuration
LLM_MODEL=mistral/mistral-small-latest  # or gpt-4o-mini, claude-3-opus, etc.
MISTRAL_API_KEY=your_key_here  # or OPENAI_API_KEY, ANTHROPIC_API_KEY, etc.
...
2. **Session-aware graph execution**  
   ...
   - suspended session -> resume with `Command(resume=...)`
...
4. **Collector phase (multi-turn)**  
   ...
   - bounded step progress (`total_steps=10`, clarification prompts do not increase total)
   - support for meaningful unknown/negative inputs (e.g., "I don't know", "no") where valid
...
- [ ] Implement production LlamaIndex + pgvector hybrid retrieval/reranking
- [ ] Enable PostgreSQL `AsyncPostgresSaver` in deployed environments
- [ ] Add debug endpoints for workflow inspection

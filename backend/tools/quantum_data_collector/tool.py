...
- Return ONLY the question text."""
        ...
        async def pick_active_field_step(state: QuantumDataCollectorState) -> QuantumDataCollectorState:
            ...
            return state
            #TODO : return state + move field line in next node
        ...
        async def step(state: QuantumDataCollectorState) -> QuantumDataCollectorState: # TODO : separate step in several nodes
            print("step ->")
...
#--------------------------------generate question---------------------------------------
            ...
            step_num = list(s["key"] for s in self.FIELD_SPECS).index(field_key) + 1
            state["step"] = step_num # TODO : to remove
            prompt_id = state.get("pending_prompt_id") or str(uuid.uuid4())
            state["pending_prompt_id"] = prompt_id
...
#----------------------------------HITL------------------------------------------------------
            ...
            print("debug step (quantum_data_collector/tool.py l.386) : interrupting")
            answer = interrupt(
                {
                    "text": question,
                    "prompt_id": prompt_id,
